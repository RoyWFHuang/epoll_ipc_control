...
bool undefineDeviceEvent(int fd_int)
{	
	for (int index_int = 0;index_int < MAX_EVENT_NUM;index_int++)
	{
		if(__g_device_event_struct[index_int].fd_int == fd_int)
		{
			...
			__g_device_event_struct[index_int].use_bool = false;
...
{	
	...
	{
		if(!__g_device_event_struct[index_int].use_bool)
		{			
			__g_device_event_struct[index_int].use_bool = true;
			break;
		}
